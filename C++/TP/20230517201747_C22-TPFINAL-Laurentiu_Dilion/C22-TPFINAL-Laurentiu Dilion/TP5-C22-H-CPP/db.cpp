...
void db_read(size_t no, Membre* m)					// att: no = numéro logique de l'enregistrement: [1..n]  où n = db_size()
{
	f.seekg((no-1) * sizeof(Membre), ios::beg);
	f.read((char*)m, sizeof(Membre));
}

void db_write(size_t no, Membre* m)				// att: no = numéro logique de l'enregistrement: [1..n]  où n = db_size()
{
	f.seekg((no-1) * sizeof(Membre), ios::beg);
