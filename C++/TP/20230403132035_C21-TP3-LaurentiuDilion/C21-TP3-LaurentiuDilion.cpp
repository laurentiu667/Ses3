...
Item* find_infoville(List* l, string nom)
{
...
string find_city_with_most_infected(List* l) {
	int test_max = 0;
	string nom_max = "Inconnu";

	for (Item* it = l->first; it; it = it->next) {
		if (test_max < it->infoville.infecter) {
			test_max = it->infoville.infecter;
...
string stat_07(Infection info[]) {
	...
	const string desinfectant[6] { "Je n’utilise pas de désinfectant", "Savon à vaisselle", "Purell", "Eau de javel", "Alcool à friction", "Inconnue"};
...
string stat_10(Repondant rep[], Infection info[]) {
	...
	for (int i = 0; i < nb_repondants; ++i) {
		...
		int nbtest = info[i].r10;
		Item* itemville = find_infoville(l, nom);

		if (itemville == nullptr) {
			infoville* v = new infoville{ nom, nbtest };
			push_last(l, *v);
		}
		else {
			itemville->infoville.infecter += nbtest;
		}
	}
	string nom_max = find_city_with_most_infected(l);
